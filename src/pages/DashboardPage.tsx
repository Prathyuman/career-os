  ...
  return (
    <PageLayout title="Executive Command Center">
      ...
      <ScrollReveal className="mb-6">
        <div className="pro-card p-6 sm:p-8 bg-white border border-slate-200 shadow-sm relative overflow-hidden">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 relative z-10">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 rounded text-xs font-mono font-bold badge-sky">
                  TARGET ROLE: {targetRole.toUpperCase()}
                </span>
                <span className="px-2.5 py-0.5 rounded text-xs font-mono font-bold badge-emerald">
                  SYSTEM READY
                </span>
              </div>
              ...
              <p className="text-slate-600 text-xs sm:text-sm max-w-2xl leading-relaxed">
                CareerOS System is actively tracking your ATS resume score, skill gaps, and mock interview practice.
              </p>
            </div>

            {/* Overall Score Progress Card */}
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 flex items-center gap-5 shrink-0 w-full sm:w-auto">
              <div className="space-y-1">
                <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider font-mono">
                  Career Readiness
                </div>
                <div className="text-3xl font-extrabold text-slate-900 font-mono">{progressScore}%</div>
              ...
              <Link
                to="/progress"
                className="px-3.5 py-2 rounded-lg bg-sky-600 hover:bg-sky-700 text-white font-semibold text-xs transition-colors shadow-xs flex items-center gap-1.5"
              >
                <span>Report</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="pro-card p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider font-mono">ATS Health</span>
              <FileText className="w-4 h-4 text-sky-600" />
            </div>
            <div className="text-2xl font-bold text-slate-900 font-mono mb-1">
              {atsScore > 0 ? `${atsScore} / 100` : 'Pending'}
            </div>
            <p className="text-xs text-slate-500">Resume scan score</p>
          </div>

          <div className="pro-card p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider font-mono">Certifications</span>
              <Award className="w-4 h-4 text-amber-600" />
            </div>
            <div className="text-2xl font-bold text-slate-900 font-mono mb-1">{certificateCount}</div>
            <p className="text-xs text-slate-500">Verified credentials</p>
          </div>

          <div className="pro-card p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider font-mono">Projects</span>
              <FolderGit2 className="w-4 h-4 text-indigo-600" />
            </div>
            <div className="text-2xl font-bold text-slate-900 font-mono mb-1">{completedProjCount} / 3</div>
            <p className="text-xs text-slate-500">Portfolio blueprints</p>
          </div>

          <div className="pro-card p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider font-mono">Mock Interview</span>
              <MessageSquare className="w-4 h-4 text-emerald-600" />
            </div>
            <div className="text-2xl font-bold text-slate-900 font-mono mb-1">3-Round AI</div>
            <p className="text-xs text-slate-500">Aptitude, DSA & Camera HR</p>
          </div>
        </div>
      </ScrollReveal>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {quickActions.map((action) => (
            ...
            >
              ...
              <div className="flex items-center justify-between text-xs font-semibold text-sky-700 pt-3 border-t border-slate-100">
